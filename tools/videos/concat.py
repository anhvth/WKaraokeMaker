...
def concat_video(video1, video2, audio, output):
    ...
    images_to_video(
        ...
        tmp_output,
        fps=video1.fps,
        output_size=(video1.width, video1.height * 2),
    )
    # Using ffmpeg concat audio and output video together
    # ffmpeg -i video.mp4 -i audio.mp3 -c:v copy -c:a aac -strict experimental output.mp4
    os.system(
        "ffmpeg -y -i {} -i {} -c:v copy -c:a aac -strict experimental {}".format(
            tmp_output, audio, output
        )
    )
...
if __name__ == "__main__":
    ...
    for video_path_1 in video_paths_1:
        audio_path = osp.join(
            args.audio_folder, osp.basename(video_path_1).replace(".mp4", ".wav")
        )
