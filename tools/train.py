...
parser.add_argument(
    ...
    default=None,
    type=str,
    help="path to pretrained checkpoint, lightning will load it automatically",
)
parser.add_argument(
    "--no-sot",
    dest="sot",
...
if __name__ == "__main__":
    ...
    dl_val = torch.utils.data.DataLoader(
        ...
        shuffle=False,
        collate_fn=lambda x: collate_fn(x, is_training=False),
    )
    ...
    dl_train = torch.utils.data.DataLoader(
        ...
        num_workers=args.num_workers,
        shuffle=True,
        collate_fn=lambda x: collate_fn(x, is_training=True),
    )
    ...
    if args.ckpt_pretrain:
        lit = CustomModelTrainer.load_from_checkpoint(
            args.ckpt_pretrain,
            model=modified_model,
            create_optimizer_fn=optim,
            create_lr_scheduler_fn=sched,
            loss_fn=nn.CrossEntropyLoss(),
        )
    else:
        lit = CustomModelTrainer(
            model=modified_model,
            create_optimizer_fn=optim,
            create_lr_scheduler_fn=sched,
            loss_fn=nn.CrossEntropyLoss(),
    ...
    trainer = get_trainer(
        ...
        monitor={"metric": "val/loss_giou", "mode": "min"},
