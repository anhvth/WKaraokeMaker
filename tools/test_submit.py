...
def convert_result_to_competion_format(
    pred_word_segments, json_path, word_idx_to_milisec_ratio
):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "exp_name", help="give a name to make it easy to find the solution.zip file"
    )
    parser.add_argument(
        "--ckpt",
        default="pretrained/whisper_karaoke_base_vn.pth",
    ...
    parser.add_argument(
        ...
        dest="sot",
        action="store_false",
        help="disable sot, by default sot is enabled",
    )
    parser.add_argument(
        "--max_samples",
        "-m",
        default=None,
        type=int,
        help="max number of samples to evaluate, for debugging purpose",
    )
    ...
    for i, path in tqdm(enumerate(json_paths)):
        item, batch = preproc(
            path, collate_fn=collate_fn_with_sot if args.sot else collate_fn_without_sot
        )
    ...
    os.system(
        f"cd outputs/{test_set}_{args.exp_name} && zip -r {test_set}_{args.exp_name}.zip submission"
    )
    print("Output: {}".format(osp.abspath(f"outputs/{test_set}_{args.exp_name}.zip")))
    print(
        "Submit -> https://challenge.zalo.ai/portal/e2e-question-answering/submission"
    )
