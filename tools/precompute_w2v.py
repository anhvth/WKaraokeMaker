...
def precompute_w2v(json_path, w2v_aligner, audio_file=None):
    ...
    cache_path = (
        item_audio_label.audio_file.replace("/songs/", "/precomputed_giou/")[:-3]
        + "pkl"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--txt_split", default="data/training/train.txt", help="Path to txt file"
    )
    parser.add_argument(
        "--ckpt",
        default="pretrained/epoch=43_val_loss=0.0000.ckpt",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Run with this option to get commands",
    )
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="Task id, when dry-run is on, this is untouch",
    )
    parser.add_argument(
        "--gpus",
        default="0,1,2,3,4,5,6,7",
        help="List of gpus used, when dry-run is on",
    )
    parser.add_argument(
        "--max_parallel_process",
        default=16,
        type=int,
        help="Num of process runing in parallel, \
    ...
    if args.dry_run:
        ...
        for group_id, cmds in enumerate(list_cmds):
            ...
            with open(tmp_file, "w") as f:
                f.write(cmd)
            print(
                f"tmux new -s '{args.name}_{group_id:02d}' -d 'sh {tmp_file} && echo Done && sleep 10'"
            )
            os.system(
                f"tmux new -s '{args.name}_{group_id:02d}' -d 'sh {tmp_file} && echo Done && sleep 10'"
            )
